...
function getPostComments(feed, i, link) {
    // 1. Lấy Tên người bình luận an toàn
    var n = 'Anonymous';
    if (feed[i].author && feed[i].author[0] && feed[i].author[0].name && feed[i].author[0].name.$t) {
        n = feed[i].author[0].name.$t;
    }
    ...
    // 3. Lấy Nội dung bình luận an toàn
    var h = 'Nội dung không khả dụng.';
    if (feed[i].title && feed[i].title.$t) {
        h = feed[i].title.$t;
    } else if (feed[i].summary && feed[i].summary.$t) {
        h = feed[i].summary.$t;
    } else if (feed[i].content && feed[i].content.$t) {
        h = feed[i].content.$t;
    }
   ...
   // Sửa lỗi thời gian bình luận
$(document).ready(function() {
    $(".date-time a").each(function() {
        var $t = $(this);
        var originalTime = $t.attr('title'); // Lấy thời gian chuẩn Blogger lưu trong title
        if (originalTime) {
            $t.text(originalTime); // Hiển thị thời gian gốc sạch đẹp
        }
    });
});
